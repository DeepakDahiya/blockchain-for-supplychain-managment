...
final public  class SHA256Digest {
    ...
    public static byte[] digest(byte[] data) {
        ...
        for (int i = 0; i < padded.length / 64; ++i) {
            ...
            for(int k = 0 ; k < 8 ; k++)
            {
                System.out.print(Integer.toHexString(registers[k]) + " ");
            }
    ...
    private static void setupWords() {
        ...
        for (int j = 16; j < 64; ++j) {
            int roh0 = (Integer.rotateRight(words[j - 15], 7) ^
                    Integer.rotateRight(words[j - 15], 18) ^
                    (words[j - 15] >>> 3) )  ;


            int roh1 = Integer.rotateRight(words[j - 2], 17) ^
                    Integer.rotateRight(words[j - 2], 19) ^
                    (words[j - 2] >>> 10);

            words[j] = words[j - 16] + roh0 + words[j - 7] + roh1;
        }
    }
    ...
    private static void iterate(int[] registers, int[] words, int j) {
        int sigma0 = Integer.rotateRight(registers[0], 2) ^
        ...
        int sigma1 = Integer.rotateRight(registers[4], 6) ^
        ...
        int temp1 = registers[7] + sigma1 + ch + K[j] + words[j];
